...
function populateRoundAnswers(
  ...
  const answersCopy = categoryQuestion[
    Object.keys(categoryQuestion)[0]
  ].slice();
...
function handleUserGuess(userGaveUp, handlerInput) {
  ...
  if (
    ...
    gameQuestions.push(currentQuestionIndex);
  ...
  Object.assign(sessionAttributes, {
    ...
    score: currentScore,
    correctAnswerText: categoryQuestion[Object.keys(categoryQuestion)[0]][0]
  });
...
const CHOOSE_CATEGORY = `Welcome to the Medical Quiz!  Choose a category to start. The categories cardiovascular, paediactrics or orthopedics.`;
const CANCEL_MESSAGE = `Thank you for using the medical quiz!  Let's play again soon!`;
const CATEGORY_PROMPT = `Which category would you like? You can choose from cardiovascular, paediactrics or orthopedics.`;
